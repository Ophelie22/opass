...
async function main() {
  ...
  const [culture, nature, historique] = await Promise.all([
    prisma.siteCategory.create({ data: { name: "Culture", regionId: regionNord.id } }),
    prisma.siteCategory.create({ data: { name: "Nature", regionId: regionSud.id } }),
    prisma.siteCategory.create({ data: { name: "Historique", regionId: regionNord.id } }),
  ]);
  ...
  const [packDecouverte, packVIP] = await Promise.all([
    prisma.package.create({
      data: { name: "Pack Découverte", regionId: regionNord.id, price: 29.99, description: "Pour explorer les sites" },
    }),
    prisma.package.create({
      data: { name: "Pack VIP", regionId: regionSud.id, price: 59.99, description: "Accès VIP pour tous les événements" },
    }),
  ]);
...
await prisma.siteUser.create({
  data: {
    ...
    password: "password",
    site: {
      connect: { id: siteA.id }, // Connecter à un site existant
    },
  },
});
...
await prisma.siteUser.create({
  data: {
    name: "Bob Site",
    email: "bob.site@example.com",
    password: "password",
    site: {
      connect: { id: siteB.id }, // Connecter à un site existant
    },
  },
});
